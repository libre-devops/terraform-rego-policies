...
_c(name) := {"resource_changes": [{
	"address": sprintf("azurerm_private_dns_resolver_inbound_endpoint.%s", [name]),
	"mode": "managed",
...
test_silent_on_good_name if {
	count(warn) == 0 with input as _c("in-ldo-uks-prd-001")
}

test_silent_when_name_unknown if {
	count(warn) == 0 with input as {"resource_changes": [{
		"address": "azurerm_private_dns_resolver_inbound_endpoint.this", "mode": "managed", "type": "azurerm_private_dns_resolver_inbound_endpoint",
		"change": {"after": {}},
	}]}
}
