...
_c(name) := {"resource_changes": [{
	"address": sprintf("azurerm_servicebus_topic.%s", [name]),
	"mode": "managed",
...
test_silent_on_good_name if {
	count(warn) == 0 with input as _c("sbt-platform-events")
}

test_silent_when_name_unknown if {
	count(warn) == 0 with input as {"resource_changes": [{
		"address": "azurerm_servicebus_topic.this", "mode": "managed", "type": "azurerm_servicebus_topic",
		"change": {"after": {}},
	}]}
}
