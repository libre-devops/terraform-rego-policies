...
_c(name) := {"resource_changes": [{
	"address": sprintf("azurerm_ssh_public_key.%s", [name]),
	"mode": "managed",
...
test_silent_on_good_name if {
	count(warn) == 0 with input as _c("sshkey-ldo-uks-prd-001")
}

test_silent_when_name_unknown if {
	count(warn) == 0 with input as {"resource_changes": [{
		"address": "azurerm_ssh_public_key.this", "mode": "managed", "type": "azurerm_ssh_public_key",
		"change": {"after": {}},
	}]}
}
