...
_c(name) := {"resource_changes": [{
	"address": sprintf("azurerm_eventhub_namespace_authorization_rule.%s", [name]),
	"mode": "managed",
...
test_silent_on_good_name if {
	count(warn) == 0 with input as _c("rule-telemetry-sender")
}

test_silent_when_name_unknown if {
	count(warn) == 0 with input as {"resource_changes": [{
		"address": "azurerm_eventhub_namespace_authorization_rule.this", "mode": "managed", "type": "azurerm_eventhub_namespace_authorization_rule",
		"change": {"after": {}},
	}]}
}
